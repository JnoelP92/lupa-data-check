  ...
  rules: [
    {
      ...
      run: (ctx, rows) => rows.filter((p) => isBlank(p.employeeId) && ['pending', 'completed'].includes(lower(p.status))).map((p) => ({
        record: p, display: label(p), fields: { petId: p.petId, status: p.status },
      })),
    },
    {
    ...
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => isBlank(p.productName)).map((p) => ({
        record: p, display: '(no product name)', fields: { petId: p.petId, sourceProductId: p.sourceProductId },
      })),
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => Number(p.quantity ?? 0) <= 0).map((p) => ({
        record: p, display: label(p), fields: { petId: p.petId, quantity: p.quantity },
      })),
    },
    {
    ...
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => isBlank(p.expiryDate)).map((p) => ({
        record: p, display: label(p), fields: { petId: p.petId, startDate: p.startDate },
      })),
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => {
        const e = parseDate(p.expiryDate);
        return lower(p.status) === 'pending' && e && e < ctx.now;
      }).map((p) => ({ record: p, display: label(p), fields: { petId: p.petId, expiryDate: p.expiryDate } })),
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => {
        ...
        return s && e && s > e;
      }).map((p) => ({ record: p, display: label(p), fields: { petId: p.petId, startDate: p.startDate, endDate: p.endDate } })),
    },
    {
      ...
      }).map((p) => ({ record: p, display: label(p), fields: { petId: p.petId, startDate: p.startDate } })),
    },
    {
    ...
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => Number(p.refillLimit ?? 0) < 0).map((p) => ({
        record: p, display: label(p), fields: { petId: p.petId, refillLimit: p.refillLimit },
      })),
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => Number(p.refillLimit ?? -1) >= 0 && dispenses(p).length > Number(p.refillLimit) + 1).map((p) => ({
        ...
        fields: { petId: p.petId, refillLimit: p.refillLimit, dispensed: dispenses(p).length },
      })),
    },
    {
    ...
    },
    {
      ...
      run: (ctx, rows) => rows.flatMap((p) => (p.prescriptionDispenses ?? [])
        ...
        .map((d) => ({ record: p, display: label(p), fields: { petId: p.petId, authorisedAt: d.authorisedAt } }))),
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => !isBlank(p.petId) && !ctx.ix.petIds.has(p.petId)).map((p) => ({
        record: p, display: label(p), fields: { danglingPetId: p.petId },
      })),
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => !isBlank(p.clientId) && !ctx.ix.clientIds.has(p.clientId)).map((p) => ({
        record: p, display: label(p), fields: { danglingClientId: p.clientId },
      })),
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => !isBlank(p.employeeId) && !ctx.ix.employeeIds.has(p.employeeId)).map((p) => ({
        record: p, display: label(p), fields: { danglingEmployeeId: p.employeeId },
      })),
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => !isBlank(p.sourceProductId) && !ctx.ix.productIds.has(p.sourceProductId)).map((p) => ({
        record: p, display: label(p), fields: { danglingProductId: p.sourceProductId },
      })),
    },
    {
      ...
      run: (ctx, rows) => rows.filter((p) => !isBlank(p.appointmentId) && !ctx.ix.appointmentIds.has(p.appointmentId)).map((p) => ({
        record: p, display: label(p), fields: { danglingAppointmentId: p.appointmentId },
      })),
    },
  ],
};
